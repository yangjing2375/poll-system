...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        switch ($type) {
            ...
            case 'poll_ranking':
                $stmt = $db->prepare("SELECT p.id, p.title, p.topic, COUNT(pv.id) as vote_count, COUNT(DISTINCT pv.user_id) as participant_count FROM polls p LEFT JOIN poll_votes pv ON p.id = pv.poll_id GROUP BY p.id ORDER BY vote_count DESC LIMIT 10");
                $stmt->execute();
            ...
            case 'user_ranking':
                $stmt = $db->prepare("SELECT u.id, u.username, u.email, COUNT(pv.id) as vote_count, COUNT(DISTINCT pv.poll_id) as poll_count FROM users u LEFT JOIN poll_votes pv ON u.id = pv.user_id GROUP BY u.id ORDER BY vote_count DESC LIMIT 10");
                $stmt->execute();
            ...
            case 'recent_votes':
                $stmt = $db->prepare("SELECT pv.id, pv.voted_at, p.title as poll_title, u.username as user_name, po.option_text FROM poll_votes pv JOIN polls p ON pv.poll_id = p.id JOIN users u ON pv.user_id = u.id JOIN poll_options po ON pv.option_id = po.id ORDER BY pv.voted_at DESC LIMIT 20");
                $stmt->execute();
